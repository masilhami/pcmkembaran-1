...
export default function CommentSection({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    if (!slug || !supabase) {
      console.error("Supabase atau Slug hilang! Cek Environment Variables di Vercel.");
      return;
    }
    fetchComments()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // DIAGNOSTIK: Ini akan muncul di layar HP/Laptop Anda
    if (!supabase) {
      alert("CRASH: Koneksi ke Supabase gagal! NEXT_PUBLIC_SUPABASE_URL belum diisi di Vercel.")
      return
    }
    ...
    const commentData = {
      ...
      is_guest: !user,
      is_approved: true, // Langsung muncul
    ...
    try {
      ...
      if (error) {
        ...
        alert("BERHASIL: Komentar terkirim!")
        setFormData({ ...formData, content: '' })
        setReplyTo(null)
        fetchComments() // Refresh manual
      }
    } catch (err: any) {
      alert("CRASH SYSTEM: " + err.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <section className="mt-16 p-8 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div ref={formRef} className="mb-12">
        {!user && !isGuest ? (
          <div className="flex flex-col items-center p-10 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
            <p className="text-slate-500 font-bold mb-5 italic text-sm">Mari berbagi pemikiran yang mencerahkan!</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 animate-in fade-in slide-in-from-top-2">
            ...
            {user && (
              <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl mb-2 border border-blue-100">
                ...
                <p className="text-xs font-black text-[#004a8e]">Berdiskusi sebagai: {user.user_metadata?.full_name}</p>
              </div>
            )}
      ...
      <div className="space-y-6">
        {comments.filter(c => !c.parent_id).map((mainComment) => (
          <div key={mainComment.id} className="comment-block">
            <div className="flex gap-4 p-5 rounded-3xl hover:bg-slate-50 transition-all border-b border-slate-50">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#ffc107] to-[#ffd54f] flex items-center justify-center font-black text-[#004a8e] shrink-0 uppercase">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[10px] font-bold text-slate-300 italic">{new Date(mainComment.created_at).toLocaleDateString('id-ID')}</span>
                </div>
                <p className="text-slate-600 text-[15px] leading-relaxed mb-3">{mainComment.content}</p>
                <button onClick={() => scrollToForm(mainComment)} className="text-[10px] font-black text-[#004a8e] hover:text-[#ffc107] uppercase tracking-widest px-3 py-1 border border-slate-100 rounded-lg">Balas</button>
              </div>
            </div>
            ...
            <div className="ml-14 mt-4 space-y-4">
              {comments.filter(reply => reply.parent_id === mainComment.id).map(reply => (
                <div key={reply.id} className="flex gap-3 p-4 bg-slate-50 rounded-2xl border-l-4 border-[#ffc107] animate-in fade-in slide-in-from-left-2">
                  <div className="w-8 h-8 rounded-lg bg-white border border-slate-100 flex items-center justify-center font-black text-[#004a8e] text-xs shrink-0 uppercase">
        ...
        {comments.length === 0 && <p className="text-center text-slate-400 py-10 italic text-sm">Belum ada diskusi.</p>}
      </div>
    </section>
  )
}